...
namespace SolarNet.Samples.GridDuel
{
    public sealed class GridDuelNetworkedDemo : MonoBehaviour
    {
        ...
        private enum TransportMode { BluetoothClassic, Nearby }
        ...
        private void OnGUI()
        {
            ...
            GUILayout.Space(6f);
            if (!_started) DrawSetup(); else DrawRunning();
            GUILayout.Space(8f);
        ...
        private async void OnPeerConnected(string peerId)
        {
            ...
            AddLog(_status);
            try { if (_room != null) await _room.NotifyPeerConnectedAsync(peerId); }
            catch (Exception ex) { AddLog("Room join error: " + ex.Message); }
        }
