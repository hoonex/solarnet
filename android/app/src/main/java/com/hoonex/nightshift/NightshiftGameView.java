...
public final class NightshiftGameView extends GLSurfaceView {
    ...
    public NightshiftGameView(Context context) {
        ...
        renderer = new NightshiftRenderer();
    ...
    @Override public boolean onTouchEvent(MotionEvent e) {
        ...
        if (action == MotionEvent.ACTION_DOWN || action == MotionEvent.ACTION_POINTER_DOWN) {
            ...
            if (x < getWidth() * 0.48f && movePointer == -1) {
                movePointer = id; moveStartX = x; moveStartY = y;
            } else if (lookPointer == -1) {
                lookPointer = id; lookLastX = x; lookLastY = y;
            }
        } else if (action == MotionEvent.ACTION_MOVE) {
            for (int i=0;i<e.getPointerCount();i++) {
                int id = e.getPointerId(i); float x = e.getX(i), y = e.getY(i);
                if (id == movePointer) {
                    float r = Math.max(90f, getWidth() * 0.11f);
                    strafe = clamp((x - moveStartX) / r, -1, 1);
                    forward = clamp((moveStartY - y) / r, -1, 1);
                } else if (id == lookPointer) {
                    yaw += (x - lookLastX) * 0.0065;
                    pitch = clamp(pitch - (y - lookLastY) * 0.0048, -0.75, 0.75);
                    lookLastX = x; lookLastY = y; renderer.setPitch((float)pitch);
                }
            }
